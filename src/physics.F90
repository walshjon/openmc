...
!===============================================================================
  ...
  subroutine elastic_scatter(i_nuclide, rxn, E, uvw, mu_lab)
    ...
    real(8) :: E_new     ! provisional final energy
    type(Nuclide), pointer :: nuc => null()
    ...
    do
      ...
      call sample_target_velocity(nuc, v_t, E, uvw, kT)
      else
        v_t = ZERO
      end if
      ...
      E_new = dot_product(v_n, v_n)

      ! Perform SCT rejection if SCT physics are active and upscatter occurred
      if (.not. sct .or. E_new < E) then
        exit
      else
        if (prn() < exp(-(E_new - E)*(ONE/nuc % kT - ONE/nuc % kTeff))) exit 
      end if
    end do
...
!===============================================================================
  ...
  subroutine sample_target_velocity(nuc, v_target, E, uvw, kT)
